...
/**
 * Beta Scroll Action API Route
 *
 * This endpoint handles scroll requests by analyzing the page text
 * and determining the best text to scroll to.
 *
 * Expected Request:
 * {
 ...
 *   words: string          // Text to scroll to on the page
 * }
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("doing scroll action", {
      responseId,
      question,
    ...
    // Make the ChatGPT call to determine the scroll action and target text
    const completion = await openai.responses.create({
      model: "gpt-5-mini",
      instructions: [
        "You are an AI scroll assistant that helps users find specific text on web pages.",
        "Your job is to analyze the user's question and the available page text to determine what to scroll to.",
        ...
        '  "actionType": "scroll",',
        '  "words": "The exact text or section to scroll to on the page"',
        "}",
        ...
        "For scrolling, identify the specific text, heading, or section the user wants to see.",
    ...
    try {
      await connection.execute(
        "INSERT INTO VoiceChats (id, voiceConversationId, messageType, content, createdAt, responseId, action, actionType) VALUES (UUID(), ?, 'scrollAI', ?, NOW(3), ?, 'true', ?)",
        [
          conversationId,
          actionResult.answer,
    ...
    console.log("done scroll action", {
    ...
    return NextResponse.json(
      {
        responseId: openaiResponseId || "",
        answer: actionResult.answer || "Scroll action completed",
        actionType: actionResult.actionType || "scroll",
        words: actionResult.words || "",
      },
      {
        headers: {
    ...
    return NextResponse.json(
      {
        ...
        responseId: "",
        answer: "An error occurred while processing the scroll action",
        actionType: "",
        words: "",
      },
      {
